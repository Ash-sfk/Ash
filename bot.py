...
async def riddle_command(client, message: Message):
    ...
    await app.storage.set_state(chat_id=message.chat.id, state={"riddle_answer": answer})

@app.on_message(filters.command("answer"))
async def answer_command(client, message: Message):
    """Reveal riddle answer"""
    state = await app.storage.get_state(chat_id=message.chat.id)
    if state and "riddle_answer" in state:
        await message.reply_text(f"✅ *Answer*: {state['riddle_answer']}")
        await app.storage.set_state(chat_id=message.chat.id, state={})
    else:
